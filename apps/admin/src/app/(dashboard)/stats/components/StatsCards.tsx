    ...
    <section className="bg-white p-6">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-900">
          평균 이벤트 현황 ({getDisplayDateRange()} 기준)
        </h2>
      </div>
      ...
      {isInitialLoad && statsLoading ? (
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${statsLoading ? 'relative' : ''}`}>
          {/* 로딩 오버레이 */}
            ...
            <div className="text-3xl font-bold text-gray-900 mb-2">
              {(stats || previousStats) ? (
                selectedEvent === '전체' ? (
                  ...
                  (stats || previousStats)!.events.length > 0 ? (
                    (((stats || previousStats)!.events.reduce((sum, event) => sum + (event.couponUsed || 0), 0) / 
                      Math.max((stats || previousStats)!.events.reduce((sum, event) => sum + (event.couponIssued || 0), 0), 1)) * 100).toFixed(1) + '%'
                  ) : '0.0%'
                ) : (
                  // 특정 이벤트 선택 시
                  selectedEventStats ? (
          ...
          <div className="bg-gray-50 rounded border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-2">
              {(stats || previousStats) ? (
                selectedEvent === '전체' ? (
                  formatNumber((stats || previousStats)!.events.reduce((sum, event) => sum + (event.totalInflow || 0), 0))
                ) : (
          ...
          <div className="bg-gray-50 rounded border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-2">
              {(stats || previousStats) ? (
                selectedEvent === '전체' ? (
                  formatNumber((stats || previousStats)!.events.reduce((sum, event) => sum + (event.couponIssued || 0), 0))
                ) : (
          ...
          <div className="bg-gray-50 rounded border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-2">
              {(stats || previousStats) ? (
                selectedEvent === '전체' ? (
                  formatNumber((stats || previousStats)!.events.reduce((sum, event) => sum + (event.couponUsed || 0), 0))
                ) : (
